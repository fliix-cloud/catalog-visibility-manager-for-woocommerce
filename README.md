...
**Text domain / WordPress.org slug:** `fliix-catalog-visibility-manager-for-woocommerce`
...
| Locale | Files |
|--------|--------|
| Template | `languages/fliix-catalog-visibility-manager-for-woocommerce.pot` |
| English (US) | `…-en_US.po` / `.mo` (reference catalog) |
| German | `…-de_DE.po` / `.mo` |

WordPress loads `languages/fliix-catalog-visibility-manager-for-woocommerce-{locale}.mo` based on **Settings → General → Site Language**.

### Contribute a language via Pull Request

1. Copy the `.pot` to `languages/fliix-catalog-visibility-manager-for-woocommerce-{locale}.po` (e.g. `fr_FR`).
